...
# Import the game class from coffee_shop_simulator module
...
if Path(CoffeeShopSimulator.SAVE_FILE).is_file():
    ...
    response = CoffeeShopSimulator.prompt("There is a saved game. Do you want to load" \
    "it? (Y/N)", True)
    if re.search("y", response, re.IGNORECASE):
        ...
        print(f"HINT: If you don't want to see this prompt again, remove the " \
              f"{CoffeeShopSimulator.SAVE_FILE} file.")
